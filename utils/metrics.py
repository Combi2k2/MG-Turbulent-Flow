...
class SmoothnessLoss(torch.nn.Module):
    ...
    def forward(self, w):
        ldudx = self.loss((w[:, 0, 1:, :] - w[:, 0, :-1, :]) / self.delta, w[:, 0, 1:, :].detach() * 0)
        ldudy = self.loss((w[:, 0, :, 1:] - w[:, 0, :, :-1]) / self.delta, w[:, 0, :, 1:].detach() * 0)
        ldvdx = self.loss((w[:, 1, 1:, :] - w[:, 1, :-1, :]) / self.delta, w[:, 1, 1:, :].detach() * 0)
        ldvdy = self.loss((w[:, 1, :, 1:] - w[:, 1, :, :-1]) / self.delta, w[:, 1, :, 1:].detach() * 0)
...
if __name__ == '__main__':
    ...
    metrics1 = [MagnitudeLoss(mse),
                SmoothnessLoss(mse),
                DivergenceLoss(mse)]
    metrics2 = [DivergenceLoss2(mse),
                VorticityLoss(mse)]

    a = torch.randn((1, 2, 20, 20))
    b = torch.randn((1, 2, 20, 20))
    
    for metric in metrics1:
        print(metric.__class__.__name__, ": ", end = "")
        print(metric(a))
